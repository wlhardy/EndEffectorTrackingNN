...
REDUCE_LABEL_SMOOTHING_MAX = 0.5
SAMPLES_PER_CLASS = 3
LEFT_CROPPING = 398
RIGHT_CROPPING = 856
...
sweep_config = {
    ...
    "parameters": {
        ...
        "weight_loss_joints": {"value": 1.0},
        "weight_loss_xy": {"value": 0.01},
    },
}
...
def train(config=None):
    with wandb.init(config=config):
        ...
        # Load model
        ee_model = model_mlp.EndEffectorPosePred(backbone_model, num_classes=config.num_classes, mpl_dim=config.mlp_dim).to(device)
        ee_model._initialize_weights()
        ...
        for epoch in range(config.epochs):
            ...
            for i, (images, joint_values) in enumerate(dataloader_train):
                # Reset the gradients
                optimizer.zero_grad()
                ...
                x_error_pixel = torch.abs(base_y - target_x)
            ...
            total_img_count = 0
            with torch.no_grad():
                for images, joint_values in dataloader_val:
                    ...
                    y_error_pixel = torch.abs(base_y - target_y)
                    # TODO WLH: Fix the pixel error calculation because it gets over 1 per image which is not expected
                    # Check if any element in x_error_pixel or y_error_pixel is greater than 1
