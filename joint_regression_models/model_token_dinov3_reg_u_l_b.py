...
class EndEffectorPosePredToken(nn.Module):
    def __init__(self, backbone, nbr_tokens=3):
        ...
        self.lower_joint_head = nn.Sequential(
            nn.Linear(embed_dim, embed_dim // 2),
            nn.ReLU(),
            nn.Linear(embed_dim // 2, embed_dim // 2),
            nn.ReLU(),
        ...
        self.upper_joint_head = nn.Sequential(
            nn.Linear(embed_dim, embed_dim // 2),
            nn.ReLU(),
            nn.Linear(embed_dim // 2, embed_dim // 2),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        base_joint_sincos = self.base_joint_head(base_joint_task_token_out).squeeze(1)
        lower_joint_task_token_out = x_norm_clstoken[:, 1:2, :]
        lower_joint_sincos = self.lower_joint_head(lower_joint_task_token_out).squeeze()
        upper_joint_task_token_out = x_norm_clstoken[:, 2:3, :]
        upper_joint_sincos = self.upper_joint_head(upper_joint_task_token_out).squeeze()
        return base_joint_sincos, lower_joint_sincos, upper_joint_sincos